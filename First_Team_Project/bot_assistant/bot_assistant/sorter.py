...
class Sorter:
    ...
    @staticmethod
    def file_list_by_folder(sorted_path):
        if os.listdir(sorted_path):
    ...
    def del_empty_dirs(self, address):
        for dirs in os.listdir(address):
            s_dir = os.path.join(address, dirs)
            if os.path.isdir(s_dir):
                self.del_empty_dirs(s_dir)
